...
- [ ] **T01** — Repo bootstrap: `go.mod`, project tree, `cmd/opps/main.go`
      with cobra root + `opps version` + ldflags wiring, Makefile, `.gitignore`.
- [ ] **T02** — CI workflow: `.github/workflows/ci.yml` (lint + gofumpt +
...
- [ ] **T06** — Prompt harness (`internal/prompt`): huh wrappers,
...
- [ ] **T13** — Opportunity prompts + CLI with inline-create:
...
- [ ] **T14** — `opps attach contact` / `opps detach contact`: secondary
...
- [ ] **T15** — Applications store + service create + `applied` event +
      `ErrActiveExists` translation. Concurrent regression test. **No CLI
      in this task.**
- [ ] **T16** — Application status transitions: every remaining row of
...
- [ ] **T17** — `opps apply` shortcut + `opps event add` app-scoped
      contextual menu.
- [ ] **T18** — `opps follow-up [<application-id>] [--blocked] [--done]`:
      no-flag = stamp `last_followed_up_at`; `--blocked` = suppress
      future staleness alerts; `--done` = clear block + restamp.
- [ ] **T19** — Application prompts + CRUD CLI: `opps add application`
      (full from-scratch with chained inline-create through opportunity),
      `list`, `show`, `update`, `rm`.
- [ ] **T20** — Compensation & application_stages stubs: tables exist,
